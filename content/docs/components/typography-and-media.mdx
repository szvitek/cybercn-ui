...
<Preview className="space-y-4">
  <Heading Tag="h1" glitched>{"Threat Assessment"}</Heading>
  <Heading Tag="h2">Sector Report</Heading>
  <Heading Tag="h3" glitched>{"Active Signal"}</Heading>
...
<Preview className="space-y-4">
  <Paragraph>{"Standard body copy for longer descriptive passages and inline links."}</Paragraph>
  <Paragraph inverse dotted scanned="horizontal">
    {"Archive fragments remain readable despite signal corruption."}
  </Paragraph>
</Preview>
...
```tsx
<Link href="/docs/components/forms" variant="green">
...
| Prop               | Type                                        | Default | Notes                                                                |
| ------------------ | ------------------------------------------- | ------- | -------------------------------------------------------------------- |
| `variant`          | `"red" \| "green" \| "purple" \| "blue"`    | `"red"` | Sets the CyberCN link accent.                                        |
| `asChild`          | `boolean`                                   | `false` | Uses `Slot.Root` for composition.                                    |
| Next.js link props | `ComponentPropsWithoutRef<typeof NextLink>` | -       | Accepts `href`, `prefetch`, `replace`, and related navigation props. |

This is a wrapper around `next/link`, so it keeps normal App Router navigation behavior.
