...
Glitch timings (`--glitched-duration-*`) and Tailwind `animate-*` utilities are documented in [Motion and effects](./motion-and-effects.mdx).  
`clip-*` utilities and border framing are summarized in [Shapes and clipping](./shapes-and-clipping.mdx).
